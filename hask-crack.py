#!/usr/bin/python
import getopt, StringIO, hashlib, sys, os, time
...
def main(argv):
  ...
  try:
      ...
      print '[*]./hash-crack.py -t <type> -h <hash> -w <wordlist>'
      print '[*]Type ./hash-crack.py -i for information'
      sys.exit(1)
  for opt, arg in opts:
      if opt == '-i':
          info()
  ...
  if not (hashType and userHash):
      print '[*]./hask-crack.py -t <type> -h <hash> -w <wordlist>'
      sys.exit()
  ...
  try:
      ...
      if (numbersBruteforce == True):
         h.hashCrackNumberBruteforce(userHash, hashType, "[*]Testing: "+verbose)
      else:
